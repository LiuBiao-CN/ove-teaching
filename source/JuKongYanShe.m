function varargout = JuKongYanShe(varargin)
% JUKONGYANSHE MATLAB code for JuKongYanShe.fig
%      JUKONGYANSHE, by itself, creates a new JUKONGYANSHE or raises the existing
%      singleton*.
%
%      H = JUKONGYANSHE returns the handle to a new JUKONGYANSHE or the handle to
%      the existing singleton*.
%
%      JUKONGYANSHE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in JUKONGYANSHE.M with the given input arguments.
%
%      JUKONGYANSHE('Property','Value',...) creates a new JUKONGYANSHE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before JuKongYanShe_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to JuKongYanShe_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help JuKongYanShe

% Last Modified by GUIDE v2.5 15-Mar-2024 22:53:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @JuKongYanShe_OpeningFcn, ...
                   'gui_OutputFcn',  @JuKongYanShe_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before JuKongYanShe is made visible.
function JuKongYanShe_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to JuKongYanShe (see VARARGIN)

% Choose default command line output for JuKongYanShe
...
% UIWAIT makes JuKongYanShe wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = JuKongYanShe_OutputFcn(hObject, eventdata, handles) 
...
if(var1 == 1)
    ...
    c=512;r=c;%矩孔的大小
    U0=zeros(r,c);
    U0(floor(r/2+1-var):floor(r/2+1+var-1),floor(c/2+1-var):floor(c/2+1+var-1))=1;%生成图像
    U0(floor(r/2+1-var):floor(r/2+1+var-1),floor(c/2+1-var):floor(c/2+1+var-1))=1;%生成图像
    lamda=(str2double(get(handles.BoChangEdit,'string')))*1e-9;
    k=2*pi/lamda;%波长、波数
    L0=(str2double(get(handles.XianShuJianJvEdit,'string')))*10^(-3);%给出衍射面的尺寸,单位:米
    d=str2double(get(handles.YuanSheJvLiEdit,'string'));%观察屏到衍射面的距离,米
...
elseif(var1 == 2)
    ...
    d=str2double(get(handles.YuanSheJvLiEdit,'string'));%屏到衍射面的距离,单位:米
    F0=exp(1i*k*d)/(j*lamda*d);
    F1=exp(1i*k/2/d.*(x0.^2+y0.^2));
    fa=fft2(U0);%衍射孔的频谱
    fF1=fft2(F1);%周秋函数的频谱
    Fuf=fa.*fF1*L0/r*L0/c;%在观察屏上的光场分布
    U=F0.*fftshift(ifft2(Fuf));
    I=U.*conj(U);%在观察屏上的光强分布
    axes(handles.axes2);%调用干涉图样坐标
    imshow(U0,[])
    axes(handles.axes3);%调用干涉图样坐标
    imshow(I,[0,max(max(I))]),colormap(gray)
end
...
axes(handles.axes3);%调用干涉图样坐标
imshow(I,[0,max(max(I))]),colormap(gray)
%imshow(max(max(I))-I,[]),colormap(gray)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles2= JuKongYanSheYuanLi;%取得相应的界面对象
